 ...
 */
package io.github.solf.extra2.logback;
 ...
 * 
 */
public final class ExternalAppSoundAppender extends AppenderBase<ILoggingEvent>
{
	...
	public void start()
	{
		if (soundPlayer != null)
		{
			...
			{
				if (audioURL != null)
					super.start(); // successful initialization
				else
					addError("Unable to initialize SoundAppender: audioURL is not set");
			}
			else
				addError("Unable to initialize SoundAppender: soundPlayer is not a valid file: " + soundPlayer);
		}
		else
			addError("Unable to initialize SoundAppender: soundPlayer not set");
	}
